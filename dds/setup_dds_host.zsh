#!/bin/zsh
alias discovery="~/innate-os/dds/discovery.zsh"
alias discovery-and-launch-sim="~/innate-os/dds/discovery-and-launch-sim.zsh"
...
DDS_CONFIG_DIR=$(dirname "$HOME/innate-os/dds/super_client_configuration.xml")
mkdir -p "$DDS_CONFIG_DIR"

export FASTRTPS_DEFAULT_PROFILES_FILE=$HOME/innate-os/dds/super_client_configuration.xml
sed -e "s/DDS_SERVER_IP/$ROS_DISCOVERY_SERVER_IP/g" \
    -e "s/DDS_SERVER_PORT/$ROS_DISCOVERY_SERVER_PORT/g" \
    $HOME/innate-os/dds/super_client_template.xml > $FASTRTPS_DEFAULT_PROFILES_FILE
