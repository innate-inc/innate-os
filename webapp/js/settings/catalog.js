  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        knobs: [
          { path: ["brain_client_node", P, "llm_model"], label: "Brain model", default: "gemini-3.6-flash", type: "string", doc: "Model powering the local brain, on whichever server the endpoint below names", subsection: "Brain" },
          { path: ["brain_client_node", P, "llm_base_url"], label: "OpenAI-compatible endpoint", default: "", type: "string", doc: "The brain speaks Chat Completions, so any OpenAI-compatible server can drive it — give its /v1 root: a LAN vLLM (http://192.168.1.20:8000/v1), Ollama (http://192.168.1.20:11434/v1), or NVIDIA (https://integrate.api.nvidia.com/v1). Its key goes in .env as LLM_API_KEY. Empty means Gemini, through the Innate proxy or GEMINI_API_KEY.", subsection: "Brain" },
          { path: ["brain_client_node", P, "llm_thinking"], label: "Thinking", default: "minimal", type: "string", doc: "Sent as reasoning_effort: minimal, low, medium or high. Blank leaves the server's own default — the default too when an endpoint is set below; minimal is measured on Gemini.", subsection: "Brain" },
          { path: ["brain_client_node", P, "llm_extra_body"], label: "Endpoint request extras", default: "", type: "string", doc: "JSON object merged into every request, for the knobs one server has and the rest do not — e.g. {\"chat_template_kwargs\": {\"enable_thinking\": false}}.", subsection: "Brain" },
          { path: ["brain_client_node", P, "memory_llm_base_url"], label: "Recall endpoint", default: "", type: "string", doc: "Where the spatial-memory search sends the remembered frames. Blank rides the brain's own endpoint and knobs; 'gemini' keeps recall on Gemini (through the Innate proxy or GEMINI_API_KEY, with its context cache) while the brain runs elsewhere; a /v1 root is a server of its own, keyed by MEMORY_LLM_API_KEY in .env.", subsection: "Memory search" },
          { path: ["brain_client_node", P, "memory_llm_model"], label: "Recall model", default: "", type: "string", doc: "Model the search asks. Blank means the brain's model on the brain's endpoint, gemini-3.6-flash on the gemini route", subsection: "Memory search" },
          { path: ["brain_client_node", P, "memory_llm_thinking"], label: "Recall thinking", default: "", type: "string", doc: "Blank means the brain's setting on the brain's endpoint, Gemini's default on the gemini route, the server's default elsewhere. Any level sends reasoning_effort: low — a pick among labeled frames gains nothing from more", subsection: "Memory search" },
          { path: ["brain_client_node", P, "memory_llm_extra_body"], label: "Recall request extras", default: "", type: "string", doc: "JSON object merged into every search request. Blank means the brain's extras on the brain's endpoint, none elsewhere", subsection: "Memory search" },
