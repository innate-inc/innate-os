  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        knobs: [
          { path: ["brain_client_node", P, "gemini_model"], label: "Brain model", default: "gemini-3.6-flash", type: "string", doc: "Model id powering the local brain: a Gemini model, or the endpoint's own model when one is set below (e.g. qwen3-vl:8b, nvidia/nemotron-3-nano-omni-30b-a3b-reasoning). Restart to apply", subsection: "Brain" },
          { path: ["brain_client_node", P, "llm_base_url"], label: "OpenAI-compatible endpoint", default: "", type: "string", doc: "Think with any /v1 Chat Completions server instead of Gemini: a vLLM or Ollama on a computer on this network (http://192.168.1.20:8000/v1, http://192.168.1.20:11434/v1) or NVIDIA's hosted API (https://integrate.api.nvidia.com/v1, key in .env as LLM_API_KEY). Empty = Gemini. Restart to apply", subsection: "Brain" },
          { path: ["brain_client_node", P, "llm_extra_body"], label: "Endpoint request extras", default: "", type: "string", doc: "JSON merged into every request to that endpoint, for server-specific switches — {\"chat_template_kwargs\": {\"enable_thinking\": false}} turns thinking off on Nemotron 3 / Qwen3 under vLLM or NIM. Restart to apply", subsection: "Brain" },
