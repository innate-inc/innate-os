...
export function createAudioOverlay(stage, ros) {
  ...
  const state = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-state"));
  const meter = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-meter"));
  const level = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-level"));
  const threshold = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-threshold"));
  const engine = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-engine"));
  const reading = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-reading"));
  const rmsReading = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-rms"));
  const sileroReading = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-silero"));
  const duckingReading = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-ducking"));
  const deviceReading = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-device"));
  const rmsTrace = /** @type {SVGPolylineElement} */ (requiredElement(root, ".audio-debug-trace-rms"));
  const sileroTrace = /** @type {SVGPolylineElement} */ (requiredElement(root, ".audio-debug-trace-silero"));
  const triggerTrace = /** @type {SVGLineElement} */ (requiredElement(root, ".audio-debug-trace-trigger"));
  const event = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-event"));
  const failure = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-failure"));
  const failureSummary = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-failure-summary"));
  const stats = /** @type {HTMLElement} */ (requiredElement(root, ".audio-debug-stats"));
  ...
  function showUnrecognized(data, phase) {
    const samples =
      utteranceSignals.length > 0
        ? utteranceSignals
        : [{ rms: Number(data.rms) || lastRms, silero: Number(data.vad_level) || lastSilero }];
  ...
  function onVad(data) {
    ...
    lastSilero = lastEngine === "silero" ? lastLevel : 0;
    ...
    sileroPaused = lastEngine === "silero" && ducking;
    if (!vendorVad) {
  ...
  function onDebug(raw) {
    ...
    if (phase === "speech_started" && data.source === "stt") {
      ...
      lastTranscribeMs = Number(data.stop_to_transcript_ms) || 0;
      showEvent(
        lastTranscribeMs
      ...
      responseTranscriptCount = Number(data.bundled_transcripts) || 0;
      transcriptToVoiceMs = Number(data.transcript_to_voice_ms) || 0;
      stopToVoiceMs = Number(data.stop_to_voice_ms) || 0;
      setFlowStep("text", "done");
      setFlowStep("reply", "done");
      setFlowStep("voice", "active");
      showEvent(
        data.transcript_to_voice_ms == null
          ? "MARS started talking"
          : `MARS started talking · ${formatDuration(transcriptToVoiceMs)} after transcript`,
      );
  ...
  const unsubscribeChat = ros.subscribe(
    ...
    (/** @type {any} */ msg) => {
      ...
      try {
        ...
        setFlowStep("text", "done");
        setFlowStep("reply", response.voice_started ? "done" : "active");
...
function requiredElement(root, selector) {
  ...
  if (element === null) throw new Error(`Missing audio debug element: ${selector}`);
  return element;
}
