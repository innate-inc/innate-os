...
/** @typedef {{ width: number, height: number, resolution: number, originX: number, originY: number, originYaw: number, frameId: string, mapHash: string, mapId: string | null, data: number[] }} KeepoutGrid */
...
export function keepoutGridFromMessage(msg) {
  ...
  return {
    ...
    mapId: mapIdentity(msg),
    data: msg.data.slice(0, width * height).map((/** @type {number} */ value) => (value >= 50 ? 100 : 0)),
  };
}

/** Identity of one loaded /map instance.
 *
 * OccupancyGrid carries no id field, so nav2's per-load `info.map_load_time`
 * is the identity: the keepout server copies the active map's whole `info`
 * into every mask it publishes, which makes this the one key both messages
 * share. `header.stamp` is NOT interchangeable — a mask is stamped when it is
 * published, not when its map was loaded.
 * @param {any} msg @returns {string | null} */
export function mapIdentity(msg) {
  const stamp = msg?.info?.map_load_time;
  const sec = Number(stamp?.sec);
  const nanosec = Number(stamp?.nanosec);
  if (!Number.isFinite(sec) || !Number.isFinite(nanosec) || (sec === 0 && nanosec === 0)) return null;
  return `${sec}:${nanosec}`;
}

/** The keepout state for the displayed map, or null while the two disagree.
 * Geometry is checked too, so a mask carrying no load time is refused rather
 * than approximated.
 * @param {KeepoutGrid | null} grid @param {string | null} mapId
 * @param {{ width: number, height: number, resolution: number, originX: number, originY: number, originYaw: number } | null} [placement] */
export function keepoutGridForMap(grid, mapId, placement = null) {
  if (!grid || !mapId || grid.mapId !== mapId) return null;
  if (!placement) return grid;
  const near = (/** @type {number} */ a, /** @type {number} */ b) => Math.abs(a - b) <= 1e-6;
  return grid.width === placement.width &&
    grid.height === placement.height &&
    near(grid.resolution, placement.resolution) &&
    near(grid.originX, placement.originX) &&
    near(grid.originY, placement.originY) &&
    near(grid.originYaw, placement.originYaw)
    ? grid
    : null;
}
