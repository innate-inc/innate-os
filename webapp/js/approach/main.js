...
// Approach debug — what the floor-approach skills see and aim at.
//
// Subscribes to /brain/approach_debug (published by approach.Debug) and draws
// the vision model's detection, the back-projected floor point, the cabinet's
// floor line, the sweet box and the live flow-tracked pixel over the frame the
// model actually saw, beside a base_link plan view: the bumper/reach squeeze,
// the fitted cabinet face and the pose look_inside_cabinet is squaring onto.
// It runs and cancels the skills too.
//
...
<header>
    ...
    <main>
      <div>
        <div class="panel">
          ...
          <div class="legend">
            ...
            <span style="color:var(--rim)">cabinet floor line</span>
      ...
      <div>
        ...
        <div class="panel" id="square-panel" hidden>
          <h2>Square up</h2>
          <div class="body"><div class="kv" id="square-kv"></div></div>
        </div>
  ...
  const WANTED = ["look_inside_cabinet", "drop_in_box", "pick_any_object"];
  ...
  const last = { detect: null, localize: null, follow: null, square: null, close: null, rim: null, release: null, hold: null, verify: null };
  ...
  function drawView() {
    ...
    if (sweet?.center_px) {
      ...
      // Rectangular, not square: range and bearing get their own tolerance
      // (one image row near the park is ~cm of range but sub-mm of bearing).
      const ov = sweet.outer_v_px ?? sweet.outer_px;
      const av = sweet.accept_v_px ?? sweet.accept_px;
      view.setLineDash([5, 4]);
      view.strokeStyle = "rgba(61,220,132,.55)"; view.lineWidth = 1.5;
      view.strokeRect(cu - sweet.outer_px, cv - ov, sweet.outer_px * 2, ov * 2);
      view.setLineDash([]);
      view.strokeStyle = "#3ddc84"; view.lineWidth = 2;
      view.strokeRect(cu - sweet.accept_px, cv - av, sweet.accept_px * 2, av * 2);
      cross(view, cu, cv, 7, "#3ddc84");
      label(view, `park x=${sweet.xy?.[0]?.toFixed(2)}`, cu - sweet.accept_px, cv - av - 2, "#3ddc84");
    }
    ...
    // The floor line is what the square-up fits its normal to, so seeing where
    // the model put its two ends is the whole diagnosis when a park comes out
    // skewed: a corner slipped onto the open door reads as a rotated cabinet.
    if (det?.floor_px?.length === 2) {
      const [a, b] = det.floor_px;
      view.strokeStyle = "#5cffe0"; view.lineWidth = 2.5;
      view.beginPath(); view.moveTo(a[0], a[1]); view.lineTo(b[0], b[1]); view.stroke();
      for (const [ex, ey] of det.floor_px) {
        view.fillStyle = "#5cffe0";
        view.beginPath(); view.arc(ex, ey, 4, 0, Math.PI * 2); view.fill();
      }
      label(view, "floor line", (a[0] + b[0]) / 2 - 26, Math.min(a[1], b[1]) - 4, "#5cffe0");
    }
  ...
  function drawPlan() {
    ...
    // The square-up's own picture: the cabinet front as two back-projected
    // floor points, and the pose the skill is driving to — arrow along the
    // heading it will end at. Square means the arrow is normal to the segment.
    // The close-in leg measures the same line, so whichever ran last owns the
    // drawing — otherwise the final straight drive is shown against the line
    // from two legs ago.
    const fit = (last.close?.t ?? -1) > (last.square?.t ?? -1) ? last.close : last.square;
    const sq = last.square;
    if (fit?.line_xy?.length === 2) {
      const [a, b] = fit.line_xy;
      plan.strokeStyle = "#5cffe0"; plan.lineWidth = 3;
      plan.beginPath(); plan.moveTo(px(a[0]), py(a[1])); plan.lineTo(px(b[0]), py(b[1])); plan.stroke();
      plan.fillStyle = "#5cffe0";
      plan.fillText("cabinet front", px((a[0] + b[0]) / 2) + 6, py((a[1] + b[1]) / 2) - 6);
    }
    if (sq?.target_xy) {
      const [tx, ty] = sq.target_xy;
      const yaw = ((sq.yaw_deg ?? 0) * Math.PI) / 180;
      const L = 0.14;
      plan.strokeStyle = "#ffcf5c"; plan.lineWidth = 2;
      plan.beginPath();
      plan.moveTo(px(tx), py(ty));
      plan.lineTo(px(tx + L * Math.cos(yaw)), py(ty + L * Math.sin(yaw)));
      plan.stroke();
      cross(plan, px(tx), py(ty), 7, "#ffcf5c", 2);
      plan.fillStyle = "#ffcf5c";
      plan.fillText(`square ${(sq.yaw_deg ?? 0).toFixed(1)}deg`, px(tx) + 10, py(ty) + 14);
    }
  ...
  function render() {
    ...
    const sq = last.square, cl = last.close;
    // Both panels are skill-shaped: the square-up rows only mean anything for
    // look_inside_cabinet, the rim/release/hold rows only for drop_in_box.
    // Hidden until their skill has actually published, so a run is never read
    // against another skill's stale dashes.
    $("square-panel").hidden = !(sq || cl);
    $("run-panel").hidden = !(r || rel || h || v);
    // 8° mirrors the skill's square_tol_deg — the records don't carry it, and
    // a threshold on the page is only a colour, never a decision.
    const SQUARE_TOL_DEG = 8;
    // The yaw error worth reading is the newest one, and the close-in leg
    // measures after every square leg has moved.
    const yaw = cl?.yaw_deg ?? sq?.yaw_deg;
    const line = cl?.line_xy ?? sq?.line_xy;
    kv($("square-kv"), [
      ["leg standoff", n(sq?.standoff, 2)],
      ["yaw err", yaw != null ? `${yaw.toFixed(1)}°` : null, Math.abs(yaw ?? 0) > SQUARE_TOL_DEG ? "warn" : "ok"],
      ["leg shift", n(sq?.shift_m)],
      ["leg target x,y", sq?.target_xy ? sq.target_xy.map((q) => n(q)).join(", ") : null],
      ["front width", line ? n(Math.hypot(line[1][0] - line[0][0], line[1][1] - line[0][1]), 2) : null],
      ["gap to front", n(cl?.gap_m, 2)],
      ["note", cl?.note ?? sq?.note, (cl?.note ?? sq?.note) ? "warn" : ""],
    ]);
    kv($("run-kv"), [
