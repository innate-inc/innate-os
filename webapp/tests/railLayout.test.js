...
test("full roster: labeled groups bracketed, standalone pages cluster", () => {
  assert.equal(
    ...
    "teleop agent nav logging |AI Lab collect datasets training profiling |Maintenance armsdk approach calibration",
  );
});
...
test("sim roster: AI Lab vanishes with its label, Maintenance keeps Arm SDK", () => {
  assert.equal(
    ...
    "teleop agent nav logging |Maintenance armsdk approach",
  );
});
