...
# First, ensure we have a clean tmux environment. `=` requires an exact session
# match and avoids touching similarly named sessions from another checkout.
...
# Restarting this process does not unload an already-open browser document; its
# polling and WebSocket clients reconnect while the environment changes.
