...
tmux send-keys -t "$SESSION_NAME:rosbridge-app" "ros2 launch maurice_sim_bringup sim_rosbridge.launch.py" C-m
echo "Started rosbridge..."
sleep 2
# Split and run app
tmux split-window -t "$SESSION_NAME:rosbridge-app" -h
tmux send-keys -t "$SESSION_NAME:rosbridge-app.1" "ros2 launch maurice_control app.sim.launch.py" C-m
echo "Started app control..."
# === Window 2: WebRTC Streamer ===
tmux new-window -t "$SESSION_NAME" -n webrtc
tmux send-keys -t "$SESSION_NAME:webrtc" "ros2 launch maurice_cam webrtc_streamer.sim.launch.py" C-m
echo "Started webrtc streamer (sim mode with compressed images)..."

# === Window 3: Nav + Brain ===
tmux new-window -t "$SESSION_NAME" -n nav-brain
tmux send-keys -t "$SESSION_NAME:nav-brain" "ros2 launch maurice_nav navigation_sim.launch.py" C-m
...
tmux select-window -t "$SESSION_NAME:rosbridge-app"
