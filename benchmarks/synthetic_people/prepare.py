...
from extracted_face import FaceLocator  # noqa: E402 -- isolated pinned PR module
...
def face_crop(im, locator):
    ...
    return im.crop(bounds), {
        "face_bounds": [h.x, h.y, h.w, h.h],
        "face_crop_bounds": bounds,
        "locator_score": h.score,
        "native_crop_size": list(im.crop(bounds).size),
    }
...
def main():
    ...
    for index in range(1, 21):
        ...
        annotations.append(
            {"character_index": index, "reference": ref_ann | ref_crop, "face_query": query_ann | query_crop}
        )
            ...
            gallery.append(
                {
                    ...
                    **save(ref_face, Path(f"assets/gallery/{index:02}.jpg"), (128, 128), 85),
                }
            )
        for size in [64, 32, 16]:
            ...
            cases.append(
                {
                    "id": f"{condition}_{index:02}",
                    "condition": condition,
                    "character_index": index,
                    "expected": NAMES.get(index, "UNKNOWN"),
                    **save(query_face, Path(f"assets/queries/{condition}_{index:02}.jpg"), (size, size), 70),
                ...
                cases.append(
                    {
                        ...
                        "character_index": index,
                        "expected": NAMES.get(index, "UNKNOWN"),
                        **save(frame, Path(f"assets/queries/{condition}_{index:02}.jpg"), (width, width * 3 // 4), 70),
                        "processing": ann,
                        "estimated_face_heights_after_resize": [
                            h.h * (width * 3 / 4) / frame.height for h in native_hits
                        ],
                    }
                )
    ...
    manifest = {
        "version": 1,
        "synthetic_only": True,
        "task": "fictional_character_identity_consistency",
        "seed": 20260910,
        "gallery": gallery,
        "cases": cases,
        "generated_sources": [
            {"path": f"assets/sheets/{name}.png", "sha256": digest(HERE / "assets/sheets" / f"{name}.png")}
            for name in LAYOUTS
        ],
    ...
    canvas = Image.new("RGB", (4 * 288, 5 * 162), "white")
    draw = ImageDraw.Draw(canvas)
    for k, (index, ref, query) in enumerate(review):
        x = (k % 4) * 288
        y = (k // 4) * 162
        canvas.paste(ref.resize((128, 128)), (x, y + 24))
        canvas.paste(query.resize((128, 128)), (x + 136, y + 24))
    ...
    print(
        "Native face-query crop range:",
        min(a["face_query"]["native_crop_size"][0] for a in annotations),
        max(a["face_query"]["native_crop_size"][0] for a in annotations),
    )
