...
def ordered_inputs(manifest, case):
    ...
    for index in case["gallery_order"]:
        ...
        items.extend([("text", f"Fictional reference character: {item['label']}"), ("image", core.image_b64(item))])
    items.extend(
        [
            ("text", "QUERY: which fictional reference character, UNKNOWN, or UNCERTAIN?"),
