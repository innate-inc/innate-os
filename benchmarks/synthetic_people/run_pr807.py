...
from extracted_face import FaceEmbedder, FaceLocator, PersonDetector, _best, crop
...
def locate_embed(image, locator, embedder):
    ...
    if not eligible:
        return None, {
            "reason": "no_face" if not hits else "below_24px",
            "detected_face_heights": [h.h for h in hits],
            "eligible_face_count": 0,
        }
    hit = max(eligible, key=lambda h: h.score)
    vector = embedder.embed(image, hit)
    return vector, {
        "reason": "embedded" if vector is not None else "embed_failed",
        "detected_face_heights": [h.h for h in hits],
        "eligible_face_count": len(eligible),
        "chosen_face_height": hit.h,
        "detector_score": hit.score,
    }
...
def main():
    ...
    config = {
        "synthetic_only": True,
        "model": "YuNet 2023mar + SFace 2021dec",
        "mode": args.mode,
        "pr_commit": source["commit"],
        "manifest_sha256": core.digest(HERE / "manifest.json"),
        "runner_sha256": core.digest(Path(__file__)),
        "extract_sha256": source["extracted_sha256"],
        "face_accept": ACCEPT,
        "face_margin": MARGIN,
        "min_face_px": MIN_FACE_PX,
        "head_fraction": 0.4 if detector else None,
        "yunet_score_threshold": 0.6,
        "enrollment": "one vector per supplied reference, no updates",
        "opencv": cv2.__version__,
        "opencv_threads": cv2.getNumThreads(),
        "machine": platform.machine(),
        "platform": platform.platform(),
        "model_and_gallery_initialization_s": init,
        "warmup_iterations": 3,
        "missing_face_protocol_label": "UNCERTAIN",
    ...
    for case in cases:
        ...
        try:
            ...
            boxes = detector.detect(image) if detector else [(0.0, 0.0, 1.0, 1.0)]
            candidates = []
            diagnostics = []
            for box in boxes:
                ...
                if vector is not None:
                    ranked = sorted(
                        [(name, float(vec @ vector)) for name, vec in gallery], key=lambda p: p[1], reverse=True
                    )
            ...
            if len(candidates) == 1:
                ...
                result.update(
                    ...
                    reason="no_person"
                    if not boxes
                    else ("below_24px" if any(d["reason"] == "below_24px" for d in diagnostics) else "no_usable_face"),
                )
            result.update(
                person_box_count=len(boxes) if detector else None,
                diagnostics=diagnostics,
                embedded_face_count=len(candidates),
                pr_identity_state="known" if result["label"] in core.LABELS else "unknown",
            )
    ...
    print(
        "Reasons:",
        dict(Counter(json.loads(line)["reason"] for line in (out / "predictions.jsonl").read_text().splitlines())),
    )
