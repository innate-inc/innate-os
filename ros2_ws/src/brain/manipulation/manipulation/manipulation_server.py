...
def create_act_config(action_dim=10, chunk_size=50, **architecture_overrides):
    ...
    return ACTConfig(
        ...
        n_action_steps=min(30, max(1, chunk_size - 5)),
        speed=1.5,
...
class ManipulationServer(Node):
    def __init__(self):
        ...
        self.current_action_dim = (
            10  # Add this to track current policy's action dimension
        )
        ...
        self.arm_state_pub = self.create_publisher(
            Float64MultiArray, "/mars/arm/commands", 10
        )
        # Service clients
        self.head_ai_position_client = self.create_client(
            Trigger, "/mars/head/set_ai_position"
        )
        ...
        self.get_logger().info(
            "Behavior server ready - pure execution engine using absolute skill directories"
        )
    ...
    def execute_behavior_callback(self, goal_handle):
        ...
        if self.execution_running:
            ...
            self.get_logger().warn(
                f"Behavior {skill_dir} requested but already running"
            )
            goal_handle.abort()
            return result
        ...
        if (
            not hasattr(goal_handle.request, "behavior_config")
            or not goal_handle.request.behavior_config
        ):
            result = ExecuteBehavior.Result()
            result.success = False
            result.message = f"No behavior_config provided for skill_dir: {skill_dir}"
            self.get_logger().error(
                f"No behavior_config provided for skill_dir: {skill_dir}"
            )
            goal_handle.abort()
            return result
        ...
        outcome, reason = self._execute_behavior(
            goal_handle, skill_dir, behavior_config
        )
        ...
        if outcome == "SUCCESS":
            ...
            goal_handle.abort()
            self.get_logger().error(
                f"Behavior {skill_dir} failed with unexpected outcome: {outcome}. {reason}"
            )
    ...
    def _execute_behavior(self, goal_handle, skill_dir, behavior_config):
        ...
        try:
            ...
            self.get_logger().info(
                f"Executing {behavior_type} behavior at: {skill_dir}"
            )
    ...
    def _execute_learned_behavior(self, goal_handle, skill_dir, behavior_config):
        ...
        try:
            ...
            progress_threshold = behavior_config["execution"].get(
            ...
            if not self._check_sensor_availability():
                self.get_logger().error(
                    "Required sensors not available. Cannot execute learned behavior."
                )
                return (
                    "FAILURE",
            ...
            self.get_logger().info(
                "Setting head to AI position for optimal camera angle"
            )
            ...
            if start_pose:
                self.get_logger().info(
                    f"Moving to start pose: {start_pose} (time: {start_pose_time}s)"
                )
            ...
            while rclpy.ok():
                ...
                if elapsed_time >= duration:
                    self.get_logger().info(
                        f"Behavior timeout reached after {elapsed_time:.2f} seconds"
                    )
                    break
                ...
                if detailed_timing:
                    ...
                    tensor_creation_times.append(
                        detailed_timing.get("tensor_creation", 0)
                    )
                    model_inference_times.append(
                        detailed_timing.get("model_inference", 0)
                    )
                ...
                if progress is None:
                    ...
                    if not self._check_sensor_availability():
                        self.get_logger().error(
                            "Required sensors became unavailable during execution"
                        )
                        self._stop_robot()
                        return (
                            "FAILURE",
                ...
                if iteration_count % timing_log_interval == 0 and len(loop_times) > 0:
                    ...
                    avg_inference = (
                        np.mean(inference_times[-timing_log_interval:]) * 1000
                    )
                    max_inference = (
                        np.max(inference_times[-timing_log_interval:]) * 1000
                    )
                    ...
                    if (
                        ...
                        avg_preprocess = (
                            np.mean(preprocess_times[-timing_log_interval:]) * 1000
                        )
                        avg_tensor = (
                            np.mean(tensor_creation_times[-timing_log_interval:]) * 1000
                        )
                        avg_model = (
                            np.mean(model_inference_times[-timing_log_interval:]) * 1000
                        )
                        avg_cpu = (
                            np.mean(cpu_transfer_times[-timing_log_interval:]) * 1000
                        )
                        avg_publish = (
                            np.mean(publish_times[-timing_log_interval:]) * 1000
                        )
            ...
            if len(loop_times) > 0:
                ...
                overall_actual_hz = (
                    1000.0 / overall_avg_loop if overall_avg_loop > 0 else 0
                )
            ...
            if end_pose:
                self.get_logger().info(
                    f"Moving to end pose: {end_pose} (time: {end_pose_time}s)"
                )
            ...
            if early_termination:
                self.get_logger().info(
                    f"Learned behavior {behavior_name} completed early due to progress threshold"
                )
                return (
                    "SUCCESS",
                    "Completed early due to progress threshold being reached",
                )
            else:
                self.get_logger().info(
                    f"Learned behavior {behavior_name} completed successfully"
                )
    ...
    def _execute_poses_behavior(self, goal_handle, skill_dir, behavior_config):
        ...
        try:
            ...
            self.get_logger().info(
                f"Poses behavior {behavior_name} completed successfully"
            )
    ...
    def _execute_replay_behavior(self, goal_handle, skill_dir, behavior_config):
        ...
        try:
            ...
            with h5py.File(file_path, "r") as h5file:
                ...
                self.get_logger().info(
                    f"Loaded {actions.shape[0]} action steps from {file_path}"
                )
            ...
            if start_pose:
                ...
                self.get_logger().info(
                    f"Moving to start pose from metadata: {start_pose} (time: {start_pose_time}s)"
                )
                if not self.call_arm_goto_service(start_pose, start_pose_time):
                    self.get_logger().error("Failed to move to start pose")
                ...
                first_arm_position = first_action[
                    0:6
                ].tolist()  # First 6 elements are arm joint positions
                self.get_logger().info(
            ...
            self.get_logger().info(
                f"Replay behavior {behavior_name} completed successfully"
            )
        ...
        try:
            ...
            if os.path.isfile(config_path):
                try:
                    ...
                    self.get_logger().warn(
                        f"Failed to parse {ACT_CONFIG_FILENAME}: {e}, falling back to inference"
                    )
            else:
                self.get_logger().info(
                    f"No {ACT_CONFIG_FILENAME} found, inferring architecture from checkpoint weights"
                )
            ...
            has_orig_mod = any(
                key.startswith("_orig_mod.") for key in state_dict.keys()
            )
            ...
            if saved_config is not None:
                ...
                if pos_embed_key in state_dict:
                    ...
                    self.get_logger().info(
                        f"Inferred chunk_size={chunk_size} from checkpoint"
                    )
            ...
            self.current_policy = ACTPolicy(
                config=policy_config, dataset_stats=dataset_stats
            ).to(self.device)
            ...
            if not load_result.missing_keys and not load_result.unexpected_keys:
                self.get_logger().info(
                    "Checkpoint loaded successfully with all keys matching"
                )
            ...
            try:
                ...
                self.get_logger().info(
                    "Note: First inference will trigger actual compilation"
                )
                # Verify compilation was applied
                if hasattr(self.current_policy, "_orig_mod"):
                    self.get_logger().info(
                        "✓ Compilation verified: model has _orig_mod attribute"
                    )
                else:
                    self.get_logger().warn(
                        "⚠ Compilation may not have been applied (no _orig_mod attribute)"
                    )
            except Exception as e:
                self.get_logger().warn(
                    f"Model compilation failed: {e}, falling back to uncompiled model"
                )
            ...
            self.get_logger().info(
                f"Policy parameters: {param_count:,} ({param_count / 1e6:.1f}M)"
            )

            # Warm-up inference (run multiple times to trigger compilation)
            self.get_logger().info(
                "Running warm-up inference (triggering compilation)..."
            )
            warmup_start = time.time()
            dummy_batch = {
                "observation.image_camera_1": torch.zeros(
                    1, 3, 224, 224, device=self.device
                ),
                "observation.image_camera_2": torch.zeros(
                    1, 3, 224, 224, device=self.device
                ),
    ...
    def _run_inference_once(self, detailed_timing=None):
        ...
        try:
            ...
            img1 = cv2.resize(
                self.latest_image1, self.image_size, interpolation=cv2.INTER_AREA
            )
            img2 = cv2.resize(
                self.latest_image2, self.image_size, interpolation=cv2.INTER_AREA
            )
    ...
    def _check_sensor_availability(self):
        ...
        if self.latest_image1 is None:
            self.get_logger().warn(
                "Camera 1 (/mars/main_camera/left/image_raw) has never received data"
            )
            return False

        if self.latest_image2 is None:
            self.get_logger().warn(
                "Camera 2 (/mars/arm/image_raw) has never received data"
            )
            return False
        ...
        if self.latest_joint_timestamp is not None:
            ...
            if time_diff > timeout_threshold:
                self.get_logger().warn(
                    f"Joint state data is stale ({time_diff:.2f}s old)"
                )
                return False
    ...
    def call_arm_goto_service(self, position, time_duration=5):
        ...
        if not position or len(position) == 0:
            self.get_logger().warn(
                "Empty position provided to arm goto service, skipping"
            )
            return True
        ...
        try:
            ...
            timeout_sec = (
                time_duration + 0.2
            )  # Small buffer for network/processing overhead
            start_wait = time.time()
            while not future.done():
                if time.time() - start_wait > timeout_sec:
                    self.get_logger().error(
                        f"Arm goto service timed out after {timeout_sec}s"
                    )
                    return False
