...
class TrainingJobTrackerNode(Node):
    ...
    def __init__(self):
        ...
        self.download_dir = Path(self.declare_parameter("download_dir", "/home/jetson1/innate-os/skills").value)
        ...
        self.logger.info(
            f"Poll intervals - Running: {self.poll_interval_running}s, "
        ...
        try:
            ...
            if self.proxy.is_available():
                ...
                token_preview = (
                    self.proxy.innate_service_key[:20] + "..."
                    if len(self.proxy.innate_service_key) > 20
                    else self.proxy.innate_service_key
                )
                self.logger.debug(
                    f"Auth key present: {bool(self.proxy.innate_service_key)}, length: {len(self.proxy.innate_service_key)}, preview: {token_preview}"
                )
            else:
        ...
        self.submit_job_srv = self.create_service(SubmitTrainingJob, "/training/submit_job", self._handle_submit_job)
        self.get_job_status_srv = self.create_service(
            GetTrainingJobStatus, "/training/get_job_status", self._handle_get_job_status
        )
        self.get_job_status_by_name_srv = self.create_service(
            GetTrainingJobStatusByName, "/training/get_job_status_by_name", self._handle_get_job_status_by_name
        )
        self.list_jobs_srv = self.create_service(ListTrainingJobs, "/training/list_jobs", self._handle_list_jobs)
        self.download_model_srv = self.create_service(
            DownloadTrainingModel, "/training/download_model", self._handle_download_model
        )
        ...
        try:
            ...
            for episode_info in episodes:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"  Episode {episode_info.get('episode_id', 'unknown')}: "
                        f"{duration_seconds:.2f}s * {data_frequency}Hz = {episode_samples} samples"
                    )

                except Exception as e:
                    self.logger.warning(
                        f"  Error parsing timestamps for episode {episode_info.get('episode_id', 'unknown')}: {e}"
                    )
                    continue

            self.logger.info(
                f"  Calculated total samples: {total_samples} from {len(episodes)} episode(s) "
    ...
    def _calculate_batch_size(self, num_samples: int) -> Tuple[int, int]:
        ...
        for world_size in [8, 4, 2]:
            ...
            self.logger.debug(
                f"  world_size={world_size}, no_of_cycles={no_of_cycles}, samples_per_worker={samples_per_worker:.2f}"
            )
            if samples_per_worker >= 4:
                max_batch = min(96, int(samples_per_worker))
                batch_size = (max_batch // 4) * 4
                self.logger.info(
                    f"  ✓ Batch size calculated: {batch_size}, world_size={world_size} (samples_per_worker={samples_per_worker:.2f})"
                )
    ...
    async def _download_model(self, job_id: str, job_status: Dict):
        ...
        try:
            async with self.proxy.training as client:
                ...
                if downloaded:
                    self.logger.info(
                        f"✓ Successfully downloaded {len(downloaded)}/{len(required_files)} required file(s) for {primitive_name}"
                    )
                    if failed_files:
    ...
    async def _poll_loop(self):
        ...
        while self._running:
            try:
                ...
                if check_completed:
                    try:
                        ...
                        if completed_jobs_to_download:
                            self.logger.info(
                                f"Found {len(completed_jobs_to_download)} completed job(s) not yet downloaded"
                            )
                ...
                self.logger.info(
                    f"Processing {len(incomplete_jobs)} incomplete job(s) and {len(completed_jobs_to_download)} completed job(s) to download"
                )
                ...
                async with self.proxy.training as client:
                    for job_data in all_jobs_to_process:
                        ...
                        if current_status == "completed" and job_data in completed_jobs_to_download:
                            if not self._is_job_downloaded(job_id, job_data):
                                self.logger.info(
                                    f"Job {job_id} appears completed but not downloaded. Checking status..."
                                )
                                try:
                                    ...
                                    if fresh_status == "completed":
                                        ...
                                        self.logger.debug(
                                            f"Job {job_id} status changed from 'completed' to '{fresh_status}'. Skipping download, will poll normally."
                                        )
    ...
    def _handle_submit_job(
        self, request: SubmitTrainingJob.Request, response: SubmitTrainingJob.Response
    ) -> SubmitTrainingJob.Response:
        ...
        try:
            ...
            if is_primitive:
                ...
                if num_samples > 0:
                    ...
                    if user_batch_size:
                        self.logger.info(
                            f"  Overriding user-provided batch_size={user_batch_size} "
                            f"with calculated batch_size={calculated_batch_size}"
                        )
                    else:
                        self.logger.info(
                            f"  Setting calculated batch_size={calculated_batch_size} (not provided by user)"
                        )

                    if user_world_size:
                        self.logger.info(
                            f"  Overriding user-provided world_size={user_world_size} "
                            f"with calculated world_size={calculated_world_size}"
                        )
                    else:
                        self.logger.info(
                            f"  Setting calculated world_size={calculated_world_size} (not provided by user)"
                        )
                    ...
                    self.logger.warning(
                        "  Could not calculate batch size from dataset, using provided values or defaults"
                    )
    ...
    def _handle_get_job_status(
        self, request: GetTrainingJobStatus.Request, response: GetTrainingJobStatus.Response
    ) -> GetTrainingJobStatus.Response:
    ...
    def _handle_get_job_status_by_name(
        self, request: GetTrainingJobStatusByName.Request, response: GetTrainingJobStatusByName.Response
    ) -> GetTrainingJobStatusByName.Response:
    ...
    def _handle_list_jobs(
        self, request: ListTrainingJobs.Request, response: ListTrainingJobs.Response
    ) -> ListTrainingJobs.Response:
    ...
    def _handle_download_model(
        self, request: DownloadTrainingModel.Request, response: DownloadTrainingModel.Response
    ) -> DownloadTrainingModel.Response:
