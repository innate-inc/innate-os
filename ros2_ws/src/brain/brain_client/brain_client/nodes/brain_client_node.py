...
class BrainClientNode(Node):
    ...
    def _build_collaborators(self) -> None:
        ...
        recall = (
            pick_model(cfg.memory_llm_model, self._proxy, logger=self.get_logger()) if cfg.memory_llm_model else route
        )
        self.memory_search = (
            MemorySearch(
                self.memory_store,
                recall.model,
                gemini_rest=recall.gemini_rest,
                model_name=recall.spec.split(":", 1)[1],
                logger=self.get_logger(),
            )
            if recall.model is not None
            else None
        )
