...
def getRectChessCorners(lines_x, lines_y):
  all_warp_corners = np.array(list(itertools.product(lines_x, lines_y)))
  ...
  return warp_corners.astype(np.float32), all_warp_corners.astype(np.float32)
...
def getOrigChessCorners(warp_corners, all_warp_corners, M_inv):
  ...
  # Ensure the bottom two points are the last two in the list
  y_sorted_corners = real_corners[np.argsort(real_corners[:, 1])]
  
  top_corners = y_sorted_corners[:2]
  bottom_corners = y_sorted_corners[2:]
  
  # Sort by x-coordinate to get tl, tr, bl, br
  top_corners = top_corners[np.argsort(top_corners[:, 0])]
  bottom_corners = bottom_corners[np.argsort(bottom_corners[:, 0])]
  
  ordered_corners = np.array([top_corners[0], top_corners[1], bottom_corners[1], bottom_corners[0]])
  
  return ordered_corners[:,:2].astype(np.float32), np.asarray(all_real_corners)[:,:2].astype(np.float32)
...
def getTileImageExact(input_img, quad_corners, tile_res=64):
  # Warp exactly to the chessboard contour without any buffer
  ideal_quad_corners = np.array([[0,0], [tile_res*8,0], [tile_res*8,tile_res*8], [0,tile_res*8]], dtype=np.float32)
  
  M = cv2.getPerspectiveTransform(quad_corners, ideal_quad_corners)
  out_img = cv2.warpPerspective(np.array(input_img), M, (tile_res*8, tile_res*8))
  return out_img, M
