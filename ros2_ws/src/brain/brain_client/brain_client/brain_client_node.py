...
class BrainClientNode(Node):
    def __init__(self):
        ...
        if not self.primitives_dict:
            self.get_logger().warn("No primitives available from skills_action_server, using fallback local loading")
    ...
    def _query_available_primitives(self):
        ...
        if not client.wait_for_service(timeout_sec=10.0):
            self.get_logger().error("Timeout waiting for /brain/get_available_skills service")
            return {}
