...
"""Skill metadata -> tool definitions (JSON Schema; each vendor's dialect is pydantic-ai's job)."""
...
_WAIT_DECLARATION = ToolDefinition(
    name=WAIT,
...
_GO_TO_POINT_IN_VIEW_DECLARATION = ToolDefinition(
...
def _declaration(name: str, meta: SkillMeta) -> ToolDefinition:
    ...
    return ToolDefinition(
        name=name, description=meta.get("guidelines") or meta["name"], parameters_json_schema=parameters
    )
