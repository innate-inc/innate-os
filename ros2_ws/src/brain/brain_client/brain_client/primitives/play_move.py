...
class PlayMove(Primitive):
    ...
    def __init__(self, logger):
        ...
        self.preferred_backend = (
            cv2.CAP_V4L2
        )  # Use V4L2 backend for better compatibility
    ...
    def _load_cartesian_poses(self):
        ...
        try:
            if not os.path.exists(poses_path):
                self.logger.error(
                    f"Cartesian poses file not found at {poses_path}. Run calibrate_chess first."
                )
                return None
            ...
            with open(poses_path, "r") as f:
                data = json.load(f)
            self.logger.info(
                f"Successfully loaded {len(data['poses'])} Cartesian poses."
            )
            return data["poses"]
        except Exception as e:
    ...
    def _initialize_camera(self):
        ...
        self.camera, self.camera_index = initialize_camera(
            self.logger, self.camera_index, self.preferred_backend
        )
    ...
    def _capture_after_image(self):
        ...
        try:
            ...
            if not success:
                self.logger.error(
                    f"❌ Failed to save after-move image to {board_state_path}"
                )
                return None

            self.logger.info(
                f"📸 Captured after-move image and updated definitive state: {board_state_path}"
            )
        ...
        try:
            ...
            if move in board.legal_moves:
                ...
                for legal_move in board.legal_moves:
                    if (
                        legal_move.from_square == from_sq
                        and legal_move.to_square == to_sq
                    ):
                        board.push(legal_move)
                        new_fen = board.fen()
                        self.logger.info(
                            f"Updated FEN with matching legal move: '{new_fen}'"
                        )
                        return new_fen
    ...
    def _execute_trajectory(
        self, joint_positions, trajectory_time=3, gripper_state=None
    ):
        ...
        if len(joint_positions) != 6:
            self.logger.error(
                f"Incorrect number of joint positions provided. Expected 6, got {len(joint_positions)}: {joint_positions}"
            )
            return False
        ...
        self.logger.info(
            f"Executing trajectory: {[f'{pos:.3f}' for pos in joint_positions]}{gripper_desc}"
        )
        ...
        if future.done():
            ...
            if response and response.success:
                self.logger.info(
                    f"Trajectory started successfully, waiting {trajectory_time + 1.5}s for completion"
                )
                time.sleep(
                    trajectory_time + 0.5
                ...
                self.logger.error(
                    f"Trajectory execution failed: {response.message if response else 'No response'}"
                )
                return False
        else:
    ...
    def _move_to_square(
        self, square, pose_type, gripper_state=None, trajectory_time=1.5
    ):
        """
        ...
        if self.ik_solution is None:
            self.logger.error(
                f"IK solution not received for {position_name} within timeout."
            )
            return False
    ...
    def execute(self, **kwargs):
        ...
        if not move_str and "kwargs" in kwargs and isinstance(kwargs["kwargs"], str):
            ...
            if kw_str.startswith("move_str="):
                ...
                self.logger.info(
                    f"Parsed move_str from unexpected format: '{move_str}'"
                )

        if not self.node:
            self.logger.error(
                "PlayMove primitive is not functional due to missing ROS node."
            )
            return (
        ...
        if not self.cartesian_poses:
            return (
                "Cartesian poses not loaded, run calibrate_chess first.",
                PrimitiveResult.FAILURE,
            )
        ...
        try:
            ...
            if new_fen is None:
                return (
                    f"Failed to update board state with move {move_str}",
                    PrimitiveResult.FAILURE,
                )
            ...
            # Validate squares
            if (
                f"{from_square}_high" not in self.cartesian_poses
                or f"{to_square}_high" not in self.cartesian_poses
            ):
            ...
            if not self._move_to_square(
                ...
                return (
                    f"Failed to close gripper at {from_square}",
                    PrimitiveResult.FAILURE,
                )
            ...
            if not self._move_to_square(
                ...
                return (
                    f"Failed to lift piece from {from_square}",
                    PrimitiveResult.FAILURE,
                )
