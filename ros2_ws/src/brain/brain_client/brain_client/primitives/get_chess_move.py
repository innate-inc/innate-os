...
class GetChessMove(Primitive):
    ...
    def _capture_image(self):
        ...
        try:
            ...
            temp_fd, temp_path = tempfile.mkstemp(
                suffix=".jpg", prefix="chess_current_"
            )
    ...
    def _parse_move_from_gemini(self, gemini_response):
        ...
        try:
            ...
            self.logger.error(
                f"❌ Could not parse move from Gemini response: {gemini_response}"
            )
            return None
        ...
        try:
            ...
            if move not in board.legal_moves:
                ...
                for legal_move in board.legal_moves:
                    ...
                    raise ValueError(
                        f"Move {move_str} is not legal in current position"
                    )
    ...
    def send_feedback(self, message):
        ...
        if self.feedback_callback:
            ...
            self.logger.warning(
                "Feedback callback not registered. Cannot send feedback."
            )
        ...
        if self.stockfish_engine is None:
            try:
                ...
                self.stockfish_engine.configure(
                    {"UCI_LimitStrength": True, "UCI_Elo": self.stockfish_elo}
                )

                self.logger.info(
                    f"Stockfish engine initialized at ELO {self.stockfish_elo}"
                )
                return True
    ...
    def execute(self, **kwargs):
        ...
        """
        self.logger.info(
            "🚀 Starting GetChessMove execution with Gemini-based move detection"
        )
        ...
        try:
            ...
            self.logger.info(
                "🖼️  Creating annotated board images using calibrated corners"
            )
            ...
            temp_before_fd, temp_before_annotated = tempfile.mkstemp(
                suffix=".jpg", prefix="annotated_before_"
            )
            temp_after_fd, temp_after_annotated = tempfile.mkstemp(
                suffix=".jpg", prefix="annotated_after_"
            )
            ...
            if not detected_move_str:
                error_msg = (
                    f"Failed to parse move from Gemini response: {gemini_response}"
                )
                self.logger.error(f"❌ {error_msg}")
                return error_msg, PrimitiveResult.FAILURE
            ...
            if not new_board_fen:
                error_msg = (
                    f"Failed to update board state with move: {detected_move_str}"
                )
                self.logger.error(f"❌ {error_msg}")
                return error_msg, PrimitiveResult.FAILURE
