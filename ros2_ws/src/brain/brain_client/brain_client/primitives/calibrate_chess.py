...
class CalibrateChess(Primitive):
    ...
    def _interpolate_and_save_poses(self):
        ...
        try:
            ...
            anchor_path = pkg_resources.resource_filename(
                "brain_client", "utils/chess/chess_anchors.json"
            )
            ...
            self.logger.info(
                f"✅ Successfully interpolated and saved {len(all_poses)} Cartesian poses to {self.cartesian_poses_path}"
            )
            return True
    ...
    def _capture_and_save_initial_image(self):
        ...
        try:
            ...
            if not success:
                self.logger.error(
                    f"❌ Failed to save initial board image to {self.initial_board_image_path}"
                )
                return None

            self.logger.info(
                f"📸 Captured and saved initial board image to: {self.initial_board_image_path}"
            )
        ...
        try:
            ...
            if success and corners is not None:
                ...
                if img is not None:
                    ...
                    # Save visualization
                    viz_path = (
                        f"/tmp/chess_corners_visualization_{int(time.time())}.jpg"
                    )
                ...
                self.logger.error(
                    f"💡 Please check the debug image at: {debug_image_path}"
                )
                self.logger.error(
                    "💡 Make sure the chessboard is visible, well-lit, and properly positioned"
                )
                self.logger.error(
                    "💡 Ensure all 4 corners of the chessboard are visible in the image"
                )
                return False
        ...
        try:
            ...
            try:
                ...
                with open(self.fen_file_path, "w") as f:
                    f.write(initial_fen)
                self.logger.info(
                    f"✅ Reset chess game state. FEN file created at {self.fen_file_path}"
                )
