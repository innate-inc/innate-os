...
class TrainingClient:
    ...
    def _get_token(self) -> str:
        ...
        if not token:
            raise ValueError("X-Innate-Token is empty after stripping. Check INNATE_SERVICE_KEY environment variable.")
    ...
    async def upload_chunk(
        ...
        headers = {"Content-Range": f"bytes {start_byte}-{end_byte}/{total_size if total_size else '*'}"}
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:  # Long timeout for large uploads
            response = await client.put(session_uri, content=data, headers=headers)
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                while uploaded_bytes < file_size:
                    ...
                    result = await self.upload_chunk(
                        session_uri=session_uri, data=chunk, start_byte=uploaded_bytes, total_size=file_size
                    )
                    ...
                    self.logger.info(
                        f"  📊 Progress: {uploaded_bytes:,}/{file_size:,} bytes ({uploaded_mb:.2f}/{total_mb:.2f} MB) - {progress:.1f}%"
                    )

            self.logger.info(
                f"✅ Upload complete: {uploaded_bytes:,} bytes ({uploaded_bytes / (1024 * 1024):.2f} MB) uploaded"
            )
            return {"job_id": job_id, "status": "uploaded", "bytes_uploaded": uploaded_bytes}
            ...
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    response = await client.put(session_uri, headers={"Content-Range": f"bytes */{file_size}"})
    ...
    async def list_jobs(
        ...
        headers = self._get_headers()
        self.logger.debug(
            f"Making request to {url} with headers: X-Innate-Token present={bool(headers.get('X-Innate-Token'))}"
        )
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                self.logger.error(
                    f"Token preview (first 20 chars): {token_value[:20] + '...' if token_value and len(token_value) > 20 else (token_value or 'None')}"
                )
    ...
    async def get_incomplete_jobs(self) -> list[dict[str, Any]]:
        ...
        incomplete = [job for job in all_jobs if job.get("status") in incomplete_statuses]
    ...
    async def get_download_url(
        ...
        try:
            ...
            try:
                ...
                pass
            self.logger.error(
                f"Failed to get download URL for {filename} (job {job_id}): {e.response.status_code}{error_detail}"
            )
    ...
    async def download_file(
        ...
        self.logger.info(
            f"  📝 Streaming in chunks of {chunk_size / (1024 * 1024):.2f} MB (not loading entire file into memory)"
        )
                    ...
                    async for chunk in response.aiter_bytes(chunk_size=chunk_size):
                        ...
                        if file_size_bytes:
                            ...
                            if downloaded_bytes % (chunk_size * 4) == 0 or downloaded_bytes == file_size_bytes:
                                ...
                                self.logger.info(
                                    f"  📊 Progress: {downloaded_bytes:,}/{file_size_bytes:,} bytes ({downloaded_mb:.2f}/{file_size_mb:.2f} MB) - {progress:.1f}%"
                                )
