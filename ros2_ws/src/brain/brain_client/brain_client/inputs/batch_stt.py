...
transcriber — ElevenLabs Scribe batch, or Gemini as one Chat Completions call
carrying the WAV. Both bias toward the ``keyterms`` vocabulary: Scribe takes a
...
def gemini_transcriber(transport: ChatTransport, model: str, language: str, keyterms: Keyterms = ()) -> Transcriber:
    def transcribe(wav: bytes) -> str:
        ...
        body: dict[str, Any] = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": [{"type": "input_audio", "input_audio": audio}, {"type": "text", "text": prompt}],
                }
            ],
            "temperature": 0,
            "reasoning_effort": "minimal",
        }
        response = transport.complete(body, TRANSCRIBE_TIMEOUT_SECS)
        # choices can be [] outright (blocked or empty response), not just absent,
        # and a blocked reply carries content: null rather than "".
