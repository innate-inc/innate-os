# SPDX-License-Identifier: Apache-2.0
# Copyright (c) 2026 Innate Inc
"""Which model, over which wire, reached how.

A setting names the model as ``provider:name`` — ``google:gemini-3.6-flash``,
``anthropic:claude-sonnet-5``, ``openai:gpt-5.4-mini`` — and each provider
has its own wire (native Gemini, Anthropic Messages, OpenAI Responses). A
bare name infers its vendor from its prefix, so the retired ``gemini-3.6-flash``
setting still works. ``openai-chat`` with ``base_url`` points the Chat
Completions wire at any OpenAI-compatible server (a LAN vLLM, Ollama, NIM).

Route: the Innate proxy when the robot has a service key (it holds the vendor
keys and passes each API through under its own service name), else the
vendor's key from the environment. Claude reverses that order until the proxy
serves ``anthropic``. sim/launcher/config.py:resolve_brain_backend predicts the
proxy-vs-key choice from the host to label the dashboard — change the
precedence here and change it there.
"""

from __future__ import annotations

import json
import os
from dataclasses import dataclass
from typing import TYPE_CHECKING

from brain_client.common.enums import StrEnum
from brain_client.llm import anthropic, gemini, openai_chat, openai_responses
from brain_client.llm.http import Http
from brain_client.llm.provider import Adapter, Provider

if TYPE_CHECKING:
    from rclpy.impl.rcutils_logger import RcutilsLogger

    from innate_proxy import ProxyClient

DEFAULT_MODEL = "google:gemini-3.6-flash"
LLM_API_KEY_ENV = "LLM_API_KEY"
TURN_TIMEOUT_SECS = 90.0

_PROVIDERS = frozenset({"google", "openai", "openai-chat", "anthropic"})
_KEY_ENV = {
    "google": "GEMINI_API_KEY",
    "openai": "OPENAI_API_KEY",
    "openai-chat": "OPENAI_API_KEY",
    "anthropic": "ANTHROPIC_API_KEY",
}
# The proxy's service path per provider (no anthropic yet); OpenAI's adapters take a "/v1" root.
_PROXY_SERVICE = {"google": "gemini", "openai": "openai/v1", "openai-chat": "openai/v1"}
_ADAPTER: dict[str, Adapter] = {
    "google": gemini.ADAPTER,
    "openai": openai_responses.ADAPTER,
    "openai-chat": openai_chat.ADAPTER,
    "anthropic": anthropic.ADAPTER,
}
_BASE_URL = {
    "google": gemini.BASE_URL,
    "openai": openai_responses.BASE_URL,
    "openai-chat": openai_chat.BASE_URL,
    "anthropic": anthropic.BASE_URL,
}


class Backend(StrEnum):
    """Which way the brain reaches its model (surfaced in health and telemetry)."""

    PROXY = "innate-proxy"
    DIRECT = "direct"
    UNCONFIGURED = "unconfigured"


@dataclass(frozen=True)
class LlmRoute:
    """A configured model, or the reason there is none."""

    spec: str  # "provider:name" as resolved
    provider: Provider | None
    backend: Backend

    @property
    def model(self) -> str:
        return self.spec.split(":", 1)[1]


def split_spec(spec: str, *, base_url: str = "") -> tuple[str, str]:
    """``"provider:name"`` -> (provider, name); a bare name infers its vendor."""
    spec = spec.strip()
    if ":" in spec:
        provider, name = spec.split(":", 1)
        if provider not in _PROVIDERS:
            raise ValueError(f"unknown LLM provider {provider!r} in {spec!r} (one of {sorted(_PROVIDERS)})")
        return provider, name
    if base_url:
        return "openai-chat", spec
    if spec.startswith("claude"):
        return "anthropic", spec
    if spec.startswith(("gpt", "o1", "o3", "o4")):
        return "openai", spec
    return "google", spec


def pick(
    spec: str,
    proxy: ProxyClient | None,
    *,
    base_url: str = "",
    extra_body: str = "",
    logger: RcutilsLogger | None = None,
) -> LlmRoute:
    provider, name = split_spec(spec or DEFAULT_MODEL, base_url=base_url)
    resolved = f"{provider}:{name}"
    extra = json.loads(extra_body) if extra_body else {}
    adapter = _ADAPTER[provider]
    key = vendor_key(provider, base_url=base_url)
    proxied = proxy is not None and proxy.is_available() and provider in _PROXY_SERVICE

    def route(http: Http, backend: Backend) -> LlmRoute:
        return LlmRoute(resolved, _provider(adapter, http, name, extra), backend)

    if base_url:
        if not key:
            return LlmRoute(resolved, None, Backend.UNCONFIGURED)
        return route(Http(base_url.rstrip("/"), headers=_bearer(key), timeout=TURN_TIMEOUT_SECS), Backend.DIRECT)
    if provider == "anthropic" and key:
        return route(Http(_BASE_URL[provider], headers=adapter_headers(provider, key)), Backend.DIRECT)
    if proxied and proxy is not None:
        return route(proxy_http(proxy, _PROXY_SERVICE[provider]), Backend.PROXY)
    if not key:
        if logger is not None:
            logger.warn(f"[Brain] no route to {resolved}: no Innate service key and no {_KEY_ENV[provider]}")
        return LlmRoute(resolved, None, Backend.UNCONFIGURED)
    return route(Http(_BASE_URL[provider], headers=adapter_headers(provider, key)), Backend.DIRECT)


def vendor_key(provider: str, *, base_url: str = "") -> str:
    if base_url:
        return os.environ.get(LLM_API_KEY_ENV, "").strip()
    if provider == "google":
        return (os.environ.get("GEMINI_API_KEY") or os.environ.get("GOOGLE_API_KEY") or "").strip()
    return os.environ.get(_KEY_ENV[provider], "").strip()


def adapter_headers(provider: str, key: str) -> dict[str, str]:
    """Each vendor's own way of taking a key."""
    if provider == "google":
        return {"x-goog-api-key": key}
    if provider == "anthropic":
        return {"x-api-key": key, "anthropic-version": anthropic.API_VERSION}
    return _bearer(key)


def proxy_http(proxy: ProxyClient, service: str) -> Http:
    """The proxy's service path, authenticating as the robot (JWT renewed on 401 by the auth flow)."""
    from auth_client.httpx_auth import InnateBearerAuth

    auth_provider = proxy.auth_provider
    base = f"{proxy.proxy_url}/v1/services/{service}"
    if auth_provider is None:
        return Http(base, headers=_bearer(proxy.token), timeout=TURN_TIMEOUT_SECS)
    return Http(base, auth=InnateBearerAuth(auth_provider), timeout=TURN_TIMEOUT_SECS)


def _provider(adapter: Adapter, http: Http, model: str, extra_body: dict) -> Provider:
    if adapter is gemini.ADAPTER:
        return gemini.GeminiProvider(adapter, http, model, extra_body)
    return Provider(adapter, http, model, extra_body)


def _bearer(key: str) -> dict[str, str]:
    return {"Authorization": f"Bearer {key}"}
