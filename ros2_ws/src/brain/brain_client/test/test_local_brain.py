...
def agent_factory(monkeypatch):
    ...
    def make(trace=None) -> tuple[BrainAgent, BrainState]:
        ...
        chat = SimpleNamespace(
            ...
            speak=lambda text, replace_pending=False, **_: spoken.append((text, replace_pending)),
            spoken=spoken,
        )
        chat.stream_speech = lambda utterance_ids=(): SpeechStreamer(chat, utterance_ids)
        agent = BrainAgent(
            node,
            state,
            config,
...
def test_speech_streamer_speaks_sentence_by_sentence():
    spoken = []
    chat = SimpleNamespace(speak=lambda text, replace_pending=False, **_: spoken.append((text, replace_pending)))
...
def test_speech_streamer_mutes_leaked_tool_narration_and_skips_noise():
    spoken = []
    chat = SimpleNamespace(speak=lambda text, replace_pending=False, **_: spoken.append(text))
    streamer = SpeechStreamer(chat)
