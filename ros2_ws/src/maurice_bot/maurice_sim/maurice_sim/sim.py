...
class MauriceBotNode(Node):
    ...
    def launch_passive_viewer(self):
        ...
        while self.viewer_handle.is_running():
            time.sleep(0.005)
            self.render_offscreen_images(
                mjr_context,
                mjv_scene,
                mjv_option,
                offscreen_camera_base,
                offscreen_camera_arm,
                base_offscreen_height,
                arm_offscreen_height,
            )
        self.get_logger().info("Viewer closed.")

    def render_offscreen_images(
        self, mjr_context, mjv_scene, mjv_option, camera_base, camera_arm, base_offscreen_height, arm_offscreen_height
    ):
        ...
        mujoco.mjv_updateScene(
            self.model, self.data, mjv_option, None, camera_base, mujoco.mjtCatBit.mjCAT_ALL, mjv_scene
        )
        t2 = time.time()
        ...
        t8 = time.time()
        mujoco.mjv_updateScene(
            self.model, self.data, mjv_option, None, camera_arm, mujoco.mjtCatBit.mjCAT_ALL, mjv_scene
        )
        t9 = time.time()
        ...
        self.get_logger().info(
            f"Base camera: updateScene={1000 * (t2 - t1):.1f}ms, render={1000 * (t3 - t2):.1f}ms, "
            + f"readPixels={1000 * (t4 - t3):.1f}ms, resize/flip={1000 * (t5 - t4):.1f}ms, "
            + f"msg_prep={1000 * (t6 - t5):.1f}ms, publish={1000 * (t7 - t6):.1f}ms"
        )
        self.get_logger().info(
            f"Arm camera: updateScene={1000 * (t9 - t8):.1f}ms, render={1000 * (t10 - t9):.1f}ms, "
