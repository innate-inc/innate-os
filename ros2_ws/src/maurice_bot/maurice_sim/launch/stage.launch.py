...
def generate_launch_description():
    ...
    set_env_var = SetEnvironmentVariable("LD_LIBRARY_PATH", "/usr/local/lib:" + os.environ.get("LD_LIBRARY_PATH", ""))

    # Launch arguments
    world_arg = DeclareLaunchArgument(
        "world", default_value=TextSubstitution(text="maurice"), description="World file name without .world extension"
    )

    stamped_arg = DeclareLaunchArgument(
        "use_stamped_velocity", default_value="false", description="Publish cmd_vel as TwistStamped instead of Twist"
    )

    def world_file_configuration(context):
        file = os.path.join(maurice_sim_directory, "worlds", context.launch_configurations["world"] + ".world")
    ...
    static_tf_lidar = Node(
        ...
        arguments=[
            "-0.0764",
            "0.0",
            "0.17165",  # Translation: X, Y, Z (in meters)
            "0",
            "0",
            "0",  # Rotation: roll, pitch, yaw (in radians)
            "base_link",
            "base_laser",  # Parent and child frames
        ],
    )
