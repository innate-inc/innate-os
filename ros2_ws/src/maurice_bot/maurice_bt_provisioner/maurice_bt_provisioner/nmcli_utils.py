...
def nmcli_add_or_modify_connection(ssid, password, priority):
    ...
    success_bind, _, stderr_bind = _run_nmcli(
        ["nmcli", "connection", "modify", ssid, "connection.interface-name", DEFAULT_WIFI_INTERFACE], use_sudo=True
    )
    if not success_bind:
...
def nmcli_scan_for_ssid(target_ssid):
    ...
    success, stdout, stderr = _run_nmcli(
        ["nmcli", "--terse", "--fields", "SSID", "device", "wifi", "list", "--rescan", "yes"], timeout=15
    )
    if not success:
