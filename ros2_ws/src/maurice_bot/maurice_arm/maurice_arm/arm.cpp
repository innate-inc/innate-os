...
public:
    MauriceArmNode() : Node("maurice_arm") {
        ...
        // Setup TF broadcasters for head camera transform
        RCLCPP_INFO(this->get_logger(), "Setting up TF broadcasters for head camera");
        ...
        // Publish static transform for head pivot axis (rotation center)
        publishHeadPivotStaticTransform();
...
private:
    ...
    /**
     * @brief Publish static transform from base_link to head_pivot
     * 
     * The head_pivot frame is at the rotation axis of the head mechanism.
     * This is a fixed position relative to base_link.
     */
    void publishHeadPivotStaticTransform() {
        geometry_msgs::msg::TransformStamped static_transform;
        static_transform.header.stamp = this->now();
        static_transform.header.frame_id = "base_link";
        static_transform.child_frame_id = "head_pivot";
        
        // Head pivot is at the base offset position (rotation axis)
        static_transform.transform.translation.x = CAM_BASE_OFFSET_X;
        static_transform.transform.translation.y = CAM_BASE_OFFSET_Y;
        static_transform.transform.translation.z = CAM_BASE_OFFSET_Z;
        
        // No rotation - pivot frame is aligned with base_link
        ...
        RCLCPP_INFO(this->get_logger(), "Published static transform: base_link -> head_pivot at [%.3f, %.3f, %.3f]",
            CAM_BASE_OFFSET_X, CAM_BASE_OFFSET_Y, CAM_BASE_OFFSET_Z);
    }
