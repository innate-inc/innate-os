...
class StereoCalibrator(Node):
    ...
    def __init__(self):
        ...
        # Create subscription
        self.subscription = self.create_subscription(Image, self.stereo_topic, self.image_callback, 10)
            ...
            try:
                ...
                if charuco_ids_left is not None and charuco_ids_right is not None:
                    ...
                    if len(common_ids) < self.min_corners:
                        self.get_logger().warn(f"Image {idx}: Not enough common corners ({len(common_ids)}). Skipping.")
                        continue
    ...
    def process_capture(self):
        ...
        if charuco_ids_left is not None and charuco_ids_right is not None:
            ...
            if len(common_ids) < self.min_corners:
                self.get_logger().warn(
                    f"Not enough common corners! Common: {len(common_ids)} (need {self.min_corners}+). Try again."
                )
                return
