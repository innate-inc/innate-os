...
def generate_launch_description():
    ...
    launch_webrtc_arg = DeclareLaunchArgument(
        "launch_webrtc", default_value="true", description="Launch the WebRTC streamer"
    )
    ...
    main_camera_width_arg = DeclareLaunchArgument(
        "main_camera_width", default_value="2560", description="Main camera capture width (full FOV)"
    )

    main_camera_height_arg = DeclareLaunchArgument(
        "main_camera_height", default_value="720", description="Main camera capture height (full FOV)"
    )

    main_camera_publish_left_width_arg = DeclareLaunchArgument(
        "main_camera_publish_left_width", default_value="640", description="Main camera publish left image width"
    )

    main_camera_publish_left_height_arg = DeclareLaunchArgument(
        "main_camera_publish_left_height", default_value="480", description="Main camera publish left image height"
    )

    main_camera_publish_stereo_width_arg = DeclareLaunchArgument(
        "main_camera_publish_stereo_width", default_value="1280", description="Main camera publish stereo image width"
    )

    main_camera_publish_stereo_height_arg = DeclareLaunchArgument(
        "main_camera_publish_stereo_height", default_value="480", description="Main camera publish stereo image height"
    )

    main_camera_fps_arg = DeclareLaunchArgument(
        "main_camera_fps", default_value="30.0", description="Main camera framerate"
    )

    main_camera_frame_id_arg = DeclareLaunchArgument(
        "main_camera_frame_id", default_value="camera_optical_frame", description="Main camera frame ID"
    )

    main_camera_jpeg_quality_arg = DeclareLaunchArgument(
        "main_camera_jpeg_quality", default_value="80", description="Main camera JPEG compression quality (1-100)"
    )

    main_camera_publish_compressed_arg = DeclareLaunchArgument(
        "main_camera_publish_compressed", default_value="true", description="Main camera publish compressed image topic"
    )
    ...
    depth_output_topic_arg = DeclareLaunchArgument(
        "depth_output_topic", default_value="/mars/main_camera/depth", description="Output depth image topic"
    )

    depth_disparity_topic_arg = DeclareLaunchArgument(
        "depth_disparity_topic", default_value="/mars/main_camera/disparity", description="Output disparity image topic"
    )

    depth_max_disparity_arg = DeclareLaunchArgument(
        "depth_max_disparity", default_value="64", description="Maximum disparity for stereo matching"
    )

    depth_publish_disparity_arg = DeclareLaunchArgument(
        "depth_publish_disparity", default_value="false", description="Publish disparity visualization image"
    )

    depth_stereo_width_arg = DeclareLaunchArgument(
        "depth_stereo_width", default_value="1280", description="Expected stereo image width (full stereo, left+right)"
    )

    depth_stereo_height_arg = DeclareLaunchArgument(
        "depth_stereo_height", default_value="480", description="Expected stereo image height"
    )
    ...
    arm_camera_width_arg = DeclareLaunchArgument(
        "arm_camera_width", default_value="640", description="Arm camera capture width"
    )

    arm_camera_height_arg = DeclareLaunchArgument(
        "arm_camera_height", default_value="480", description="Arm camera capture height"
    )

    arm_camera_fps_arg = DeclareLaunchArgument(
        "arm_camera_fps", default_value="30.0", description="Arm camera framerate"
    )

    arm_camera_pixel_format_arg = DeclareLaunchArgument(
        "arm_camera_pixel_format", default_value="YUYV", description="Arm camera pixel format"
    )

    arm_camera_publish_compressed_arg = DeclareLaunchArgument(
        "arm_camera_publish_compressed", default_value="true", description="Arm camera publish compressed image topic"
    )
