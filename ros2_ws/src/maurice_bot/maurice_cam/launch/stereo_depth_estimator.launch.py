...
def generate_launch_description():
    ...
    frame_id_arg = DeclareLaunchArgument(
        "frame_id", default_value="camera_optical_frame", description="TF frame ID for output messages"
    )
    ...
    publish_disparity_arg = DeclareLaunchArgument(
        "publish_disparity", default_value="false", description="Publish disparity visualization image"
    )
    ...
    stereo_height_arg = DeclareLaunchArgument(
        "stereo_height", default_value="480", description="Expected stereo image height"
    )
    ...
    publish_pointcloud_arg = DeclareLaunchArgument(
        "publish_pointcloud", default_value="true", description="Publish 3D point cloud from depth"
    )

    pointcloud_topic_arg = DeclareLaunchArgument(
        "pointcloud_topic", default_value="/mars/main_camera/points", description="Point cloud output topic"
    )
    ...
    bm_window_size_arg = DeclareLaunchArgument(
        "bm_window_size", default_value="11", description="Block matching window size (odd number, typically 5-15)"
    )
