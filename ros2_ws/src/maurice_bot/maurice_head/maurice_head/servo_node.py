...
class HeadServoNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"Position offset: {self.position_offset} encoder counts")
    ...
    def _configure_servo(self, pwm_limit, current_limit):
        ...
        try:
            ...
            min_encoder = int((min_servo_rad / (2 * math.pi)) * 4096 + 2048 + self.position_offset)
            max_encoder = int((max_servo_rad / (2 * math.pi)) * 4096 + 2048 + self.position_offset)
            
            self.get_logger().info(f"Setting position limits: {min_encoder} (min) to {max_encoder} (max)")
    ...
    def _logical_angle_to_encoder(self, logical_angle):
        ...
        encoder_value = int((angle_rad / (2 * math.pi)) * 4096 + 2048 + self.position_offset)
    ...
    def _move_to_logical_angle(self, logical_angle):
        ...
        try:
            ...
            self.get_logger().info(f"Moved to logical angle: {logical_angle} (encoder: {encoder_value})")
        except Exception as e:
            self.get_logger().error(f"Failed to move to logical angle {logical_angle}: {str(e)}")
