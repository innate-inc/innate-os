...
def tune_contacts(robot_spec: mujoco.MjSpec) -> None:
    ...
    # The real claw's hard stop sits past the nominal zero: closed on air the
    # encoder reads GRIPPER_CLOSED_ON_AIR_RAD (blade flex; pick_any_object's
    # GRIPPER_EMPTY_J6 is this same number). Widen the sim joint to that stop
    # so an air-close settles exactly where hardware reports it, and so
    # core.set_joint_target has a real limit to clamp the overclose command
    # into — unclamped, "-close_strength" is a position target 0.6 rad past
    # the stop and the excluded-from-colliding blades scissor straight through
    # each other and through whatever they hold.
