...
# Full dimensions and radians, shared over the opening observer frame.  Three
# builds these exact primitives; _part_xml below converts the same descriptors
# to MJCF half-sizes and degrees.
CAR_MODEL = {
    "forward_axis": "+x",
    "up_axis": "+z",
    "length": CAR_LENGTH_M,
...
def phase_at(sim_time: float) -> tuple[str, dict[str, str], dict[str, float]]:
    """Phase, group aspects, and seconds of green remaining per group."""
    t = sim_time % CYCLE_S
    if t < ALL_RED_S:
        phase = "all_red_to_ns"
        aspects = {NS: RED, EW: RED}
        remaining = {NS: 0.0, EW: 0.0}
    elif t < ALL_RED_S + GREEN_S:
        phase = "ns_green"
        aspects = {NS: GREEN, EW: RED}
        remaining = {NS: ALL_RED_S + GREEN_S - t, EW: 0.0}
    elif t < ALL_RED_S + GREEN_S + YELLOW_S:
        phase = "ns_yellow"
        aspects = {NS: YELLOW, EW: RED}
        remaining = {NS: 0.0, EW: 0.0}
    elif t < 2 * ALL_RED_S + GREEN_S + YELLOW_S:
        phase = "all_red_to_ew"
        aspects = {NS: RED, EW: RED}
        remaining = {NS: 0.0, EW: 0.0}
    elif t < 2 * ALL_RED_S + 2 * GREEN_S + YELLOW_S:
        phase = "ew_green"
        aspects = {NS: RED, EW: GREEN}
        remaining = {NS: 0.0, EW: 2 * ALL_RED_S + 2 * GREEN_S + YELLOW_S - t}
    else:
        phase = "ew_yellow"
        aspects = {NS: RED, EW: YELLOW}
        remaining = {NS: 0.0, EW: 0.0}
    return phase, aspects, remaining
...
def _part_xml(part: dict[str, Any], body_color: str, _index: int) -> str:
...
def _collider_xml(collider: dict[str, Any], _index: int) -> str:
...
class TrafficController:
    ...
    def __init__(self, environment_id: str):
        ...
        self._current_phase, initial_aspects, _remaining = phase_at(0.0)
    ...
    def bodies_xml(self) -> str:
        ...
        for car in self.cars:
            ...
            visuals = "\n".join(_part_xml(part, car.lane.color, index) for index, part in enumerate(CAR_MODEL["parts"]))
            colliders = "\n".join(
                _collider_xml(collider, index) for index, collider in enumerate(CAR_MODEL["colliders"])
            )
    ...
    def _effective_signals(self, sim_time: float) -> tuple[str, dict[str, str]]:
        ...
        """
        phase, scheduled, _remaining = phase_at(sim_time)
    ...
    def state(self, sim_time: float, world_epoch: int) -> dict[str, Any] | None:
        if not self.enabled:
            return None
