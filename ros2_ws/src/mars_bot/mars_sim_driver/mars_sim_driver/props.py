...
class Prop:
    ...
    Sidecars generated by sim/tools/roblox_to_innate.py (the bundle props)
    share one convention: the visual mesh is the full multi-primitive shape
    and the collider is a primitive around its body; the contact parameters
    are these defaults, untuned against the gripper (see sim/props/11_can.py
    for what tuned numbers look like); and drop_z is the height the object
    was authored at plus 1 cm rather than rest_z -- Drop carries no z, so a
    prop meant to sit on a plinth would otherwise be released below it and
    get shoved onto the floor, and releasing from just above its own support
    also avoids the bounce a long fall onto hard ground causes.
    """
    ...
    def collision_pieces(self) -> list[Path]:
        ...
        if mesh is None or self.collision != "pieces":
            return []
        pieces = [p for p in mesh.parent.glob(f"{mesh.stem}_collision_*") if p.suffix in (".obj", ".stl")]
...
def load_props(roots: list[Path]) -> dict[str, Prop]:
    ...
    out the world."""
    return load_sidecars(roots, "sim_prop", "PROP", "props", key=lambda p: p.name, set_root=True)
