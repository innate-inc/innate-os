...
Quick interactive demos that prove the proxy pipeline works end-to-end.
Both read `INNATE_PROXY_URL`, `INNATE_AUTH_URL`, and `INNATE_SERVICE_KEY` from
the environment (same `.env` as the other clients).

### Cartesia TTS

Synthesises text and plays the resulting WAV through `aplay`/`paplay`/`ffplay`:

```bash
# From the repo root:
PYTHONPATH=clients/proxy-client python -m demos.cartesia_tts "Hello from the robot!"

# Uses default text if no argument is given:
...
```bash
# Single question:
PYTHONPATH=clients/proxy-client python -m demos.openai_chat "What is the capital of France?"
...
| `INNATE_AUTH_URL` | For OIDC | Auth service URL for service-key → JWT exchange |
| `OPENAI_MODEL` | Optional | Model for the OpenAI chat demo (default `gpt-4o-mini`) |
