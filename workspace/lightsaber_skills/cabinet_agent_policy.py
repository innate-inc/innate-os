...
class ModelActionRejected(ValueError):
    """A tool action was rejected and corrective feedback is in history."""
...
class CabinetPolicy:
    def __init__(
        ...
        *,
        ...
        self.key = (api_key if api_key is not None else os.environ.get("OPENAI_API_KEY", "")).strip()
        self.proxy = None
        if transport is None:
            if use_proxy:
                ...
                if client.is_available():
                    self.proxy = client
            if self.proxy is None and not self.key:
                raise ValueError(
                    "Configure INNATE_SERVICE_KEY, or OPENAI_API_KEY for local development"
                    if use_proxy
                    else "Configure a direct OpenAI API key for the lightsaber skill"
                )
    ...
    def _post(self, payload):
        if self.proxy is not None:
            # Some deployed proxies forward compressed bytes after stripping
            # Content-Encoding. Request identity so the robot receives JSON.
            self.proxy.get_sync_client().headers["Accept-Encoding"] = "identity"
            try:
    ...
    def decide(self, observation, images, sleep):
        ...
        if self.optimize_cache:
            ...
            # Preserve observations in their original place beside their telemetry.
        ...
        payload = {
            ...
            "input": history,
            "tools": [self.tool],
            "tool_choice": {"type": "function", "name": self.tool["name"]},
        ...
        if self.optimize_cache:
            payload.update(prompt_cache_key=self.cache_key,
        ...
        if len(calls) != 1 or calls[0].get("name") != self.tool["name"]:
            raise ValueError(f"Expected exactly one {self.tool['name']}; no action executed")
        self.history.extend(output)  # Preserve reasoning items for the next turn.
        call = calls[0]
        try:
            action = self.validator(json.loads(call["arguments"]))
        except (ValueError, TypeError, KeyError) as error:
            reason = str(error)
            self.result(call["call_id"], f"Rejected action: {reason}. No motion executed. Correct the arguments using the tool schema and the next fresh observation.")
            raise ModelActionRejected(f"Malformed model action; no motion executed: {reason}") from None
