...
class LocalPersonEncoder:
    ...
    def _face_area(face) -> int:
        return max(0, face.location[2] - face.location[0]) * max(0, face.location[3] - face.location[1])
...
def profile_scores(profiles: list[dict[str, Any]], embeddings: dict[str, Any]) -> list[dict[str, Any]]:
...
def _rank(scores: list[dict[str, Any]], key: str) -> tuple[dict[str, Any] | None, float]:
    ranked = sorted(
...
def identity_decision(
    ...
    face_match = (
        face_best
        if face_best is not None and face_best["face_score"] >= FACE_MATCH_THRESHOLD and face_margin >= FACE_MARGIN
        else None
    )
    body_match = (
        body_best
        if body_best is not None and body_best["body_score"] >= BODY_MATCH_THRESHOLD and body_margin >= BODY_MARGIN
        else None
    )
    evidence = {
        "face_score": round(float(face_best["face_score"]), 3) if face_best is not None else None,
        "body_score": round(float(body_best["body_score"]), 3) if body_best is not None else None,
    ...
    plausible = sorted(
        {
            entry["encounter_id"]
            for entry in scores
            if (entry.get("face_score") is not None and entry["face_score"] >= FACE_PLAUSIBLE_THRESHOLD)
            or (entry.get("body_score") is not None and entry["body_score"] >= BODY_PLAUSIBLE_THRESHOLD)
        }
    )
