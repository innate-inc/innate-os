...
class PullHeldHandle(Skill):
    ...
    cancellation, or the requested travel distance.
    ...
    def execute(
        ...
        if not ready_pose or not ready_joints or direction[0] > _READY_MAX_DIRECTION_X:
            self._stop_and_fail(
                ...
                pose=self._pose_data(starting_pose),
                arm_joints=list(starting_joints),
                direction=list(direction),
                ready_bounds={
                    "x_m": [_READY_MIN_X_M, _READY_MAX_X_M],
