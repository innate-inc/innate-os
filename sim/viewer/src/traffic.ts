...
function isEmissive(material: THREE.Material): material is EmissiveMaterial {
  return isColored(material) && "emissive" in material && (material as { emissive?: unknown }).emissive instanceof THREE.Color;
}
...
export class TrafficLibrary {
  ...
  #signalMaterials = new Map<string, Map<TrafficAspect, Set<THREE.Material>>>();
  ...
  get visibleRoots(): THREE.Group[] {
    return [...this.#cars.values()].map(({ root }) => root).filter((root) => root.visible);
  }
  ...
  #collectSignalMaterials(
  ...
  ): Map<string, Map<TrafficAspect, Set<THREE.Material>>> {
    const indexed = new Map<string, Map<TrafficAspect, Set<THREE.Material>>>();
    ...
    for (const material of this.#environmentMaterials) {
      ...
      const aspects = indexed.get(group) ?? new Map<TrafficAspect, Set<THREE.Material>>();
      const materials = aspects.get(aspect) ?? new Set<THREE.Material>();
    ...
    for (const [group, aspects] of this.#signalMaterials) {
      ...
      for (const [aspect, materials] of aspects) {
        ...
        for (const material of materials) {
          if (!isColored(material)) continue;
          material.color.copy(active ? new THREE.Color(colors[aspect]) : OFF_COLOR);
          if (isEmissive(material)) {
            material.emissive.copy(active ? new THREE.Color(colors[aspect]) : OFF_COLOR);
  ...
  #assertSignalMaterials(
    ...
    indexed: Map<string, Map<TrafficAspect, Set<THREE.Material>>>,
  ): void {
    if (!manifest) return;
