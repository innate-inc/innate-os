...
test("embedding bridge follows real roster and view transitions, rejects foreign commands, and recovers", () => {
  ...
  Object.assign(globalThis, { window: win, document: { referrer: "https://sim.innate.bot/session", body: {classList: {contains: () => onboardingActive}}, addEventListener: (_: string, cb: any) => visibilityListeners.add(cb), removeEventListener: (_: string, cb: any) => visibilityListeners.delete(cb) } });
  ...
  try {
    ...
    assert.equal(state().onboardingActive, true);
