...
function mergeStaticParts(root, person) {
  // Residents use material colors; discard unused UVs so sculpted and primitive
  // surfaces can share one merged mesh. MuJoCo gets its own palette UVs below.
  root.traverse((node) => {
    if (node.isMesh) node.geometry.deleteAttribute("uv");
  });
  ...
  root.traverse((node) => {
    ...
    // Coincident vertices at closed poles have no incident triangle area.
    // Give these unused normals a unit fallback, as required by glTF.
...
// points each triangle at a swatch center, retaining subtle vertex-color
// shading from the GLB without large texture maps or atlas bleed.
function palettePNG(colors) {
  const cells = Math.ceil(Math.sqrt(colors.length));
  const width = cells * 8,
    height = cells * 8;
  ...
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
      (
        colors[Math.floor(y / 8) * cells + Math.floor(x / 8)] ?? colors[0]
      ).forEach((v, c) => {
...
function writePhysicsModel(root, id) {
  ...
  for (const mesh of meshes) {
    ...
    for (let i = 0; i < (geo.index?.count ?? p.count); i += 3) {
      const local = [0, 1, 2].map((k) =>
        geo.index ? geo.index.getX(i + k) : i + k,
      );
      const color = mesh.material.color.clone();
      if (mesh.material.vertexColors && geo.attributes.color) {
        const attr = geo.attributes.color;
        color.multiply(
          new THREE.Color(
            local.reduce((sum, k) => sum + attr.getX(k), 0) / 3,
            local.reduce((sum, k) => sum + attr.getY(k), 0) / 3,
            local.reduce((sum, k) => sum + attr.getZ(k), 0) / 3,
          ),
        );
      }
      const rgb = color
        .convertLinearToSRGB()
        .toArray()
        .map((v) => Math.round(THREE.MathUtils.clamp(v, 0, 1) * 127) * 2);
      const key = rgb.join(",");
      if (!palette.has(key)) {
        palette.set(key, colors.length + 1);
        colors.push(rgb);
      }
      const uv = palette.get(key),
        indices = local.map((k) => k + offset);
  ...
  const cells = Math.ceil(Math.sqrt(colors.length));
  const uvs = colors.map(
    (_, i) =>
      `vt ${((i % cells) + 0.5) / cells} ${1 - (Math.floor(i / cells) + 0.5) / cells}`,
  );
  writeFileSync(
    `${out}/humans/resident_${id}.obj`,
    [...uvs, ...lines].join("\n") + "\n",
  );
  writeFileSync(
