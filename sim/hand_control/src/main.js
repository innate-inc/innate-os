...
const calibration = new GroundCalibration(),
  reacquire = new Calibration(240, 4),
...
async function initTracker() {
  ...
  worker.onmessage = ({ data }) => {
    ...
    if (!calibrated) {
      ...
      feedback = sample.valid ? result.reason : sample.reason;
      if (result.ready) {
        calibrated = true;
        mapper.setGround(result.bounds);
        phase = "ready";
        feedback = "Ground set. Raise your hand to lift the arm.";
      }
    } else if (armed) {
      if (!sample.valid) {
        ...
        if (phase === "following") {
          const value = mapper.map(sample, now);
          if (value) {
            const ok = send({
              op: "move",
              token,
                ...
                value.position[1] < -0.97
                  ...
                  : Math.max(...value.position.map(Math.abs)) > 0.97
                    ...
                    : !sample.gripValid
                      ...
                      : "Thumb + index: together to grip, apart to release.";
            }
          }
        }
      }
...
function recenter() {
  ...
  calibration.reset();
  phase = "calibrating";
  feedback = "Lower your palm to the ground line and hold steady.";
  render();
}
...
function render() {
  ...
  const labels = {
    ...
    following: [
      "Following your hand",
      "Reach in 3D. Thumb + index control the grip.",
      "Pause following",
    ],
    holding: [
