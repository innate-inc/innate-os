...
SIM_WEBAPP_BIND = os.environ.get("SIM_WEBAPP_BIND", "").strip() or "127.0.0.1"
if SIM_WEBAPP_BIND not in {"127.0.0.1", "0.0.0.0"}:
    print("Error: SIM_WEBAPP_BIND must be 127.0.0.1 or 0.0.0.0.", file=sys.stderr)
    raise SystemExit(1)
# Injected into compose's environment, so a base-only override reaches the
# compose file without it knowing the base exists.
PUBLISHED_PORT_ENV = {
    "SIM_WEBAPP_BIND": SIM_WEBAPP_BIND,
...
ENVIRONMENT_CONTROL_INTENT_LOCK_PATH = ENVIRONMENT_CONTROL_DIR / "intent.lock"
ENVIRONMENT_CONTROL_SINGLETON_LOCK_PATH = ENVIRONMENT_CONTROL_DIR / "controller.lock"
ENVIRONMENT_CONTROL_STOP_PATH = ENVIRONMENT_CONTROL_STATUS_DIR / "stopping"
...
ROS_INSTALL_VALIDATION_INPUT_FILES = (
    ...
    "scripts/seed_sim_environment.py",
...
# The compatibility-floor subtrees of the image's `work` layer. The runtime
# installs every root present in the layer (so environment packs need no
# launcher edit), while these lists retain the apartment-specific guarantees:
# derived units are mandatory and authored props are optional/additive.
# tests/test_assets_image_inputs.py holds these plus manifest-selected roots
# equal to the dockerignore allowlist.
#
...
def get_config() -> dict[str, object]:
    ...
    return {
        ...
        # Validated and expanded by environment.select_environment before
        # `up` touches Docker or starts any part of the runtime.
