    ...
    *,
    ...
    try:
        ...
        if config["brain_backend"] == NO_BACKEND:
            warn("No LLM configured — the sim is running WITHOUT an agent.")
            warn(
                "Add GEMINI_API_KEY (your own Gemini key) or INNATE_SERVICE_KEY (Innate proxy) to "
                f"{ENV_PATH}, run `{CLI_SIM} setup`, or point Settings → AI models at an "
                "OpenAI-compatible server (llm_base_url), then restart."
            )
