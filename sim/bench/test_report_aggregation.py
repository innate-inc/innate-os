...
def ep(challenge, agent, passed, **kw):
    return {
        "map": "gallery", "challenge": challenge, "agent": agent, "passed": passed,
        "goals_done": 2 if passed else 0, "goals_total": 2, "elapsed_s": 10.0,
        "reason": "", "wall_s": 1.0, "steps": 10, "needs": "nav", "error": "",
        "blocked": "", "turns": 5, "path_len_m": 3.0, **kw,
    }
...
def test_the_newest_file_owns_a_challenge(results, capsys):
    ...
    assert "VALID 0" in out and "INVALID 1" in out, (
        f"an old passing oracle outvoted the newer failure:{chr(10)}{out}"
    )
...
def test_files_covering_different_challenges_are_all_used(results, capsys):
    write(results / "bench_a.json", [ep("gallery_ring_tour", "oracle", True),
                                     ep("gallery_ring_tour", "random", False)], 1_000_000)
    write(results / "bench_b.json", [ep("gallery_count_ring", "oracle", True),
                                     ep("gallery_count_ring", "random", False)], 2_000_000)
    out = run(capsys)
...
def test_separate_sweeps_are_called_a_composite(results, capsys):
    write(results / "bench_a.json", [ep("gallery_ring_tour", "oracle", True),
                                     ep("gallery_ring_tour", "random", False)], 1_000_000)
    write(results / "bench_b.json", [ep("gallery_count_ring", "oracle", True),
                                     ep("gallery_count_ring", "random", False)], 1_000_000 + 90_000)
