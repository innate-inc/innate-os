#!/usr/bin/env python3
"""
User listener — record from the mic and transcribe via Innate proxy.

Two recording modes:

* **Fixed duration** (default): ``arecord`` for ``duration_seconds`` then transcribe.
* **Silence end** (``end_on_silence=True``): stream PCM, calibrate a noise RMS level, detect speech,
  then stop after ``silence_ms_after_speech`` of quiet audio (like a push-to-talk tail), capped by
  ``max_duration_seconds``.

Programmatic API: :func:`listen_for_transcript` returns ``(text, error)``.
As a skill, success result ``message`` is JSON: ``{"transcript": "<heard text>"}``.
"""
...
def _detect_alsa_capture_device(logger: Any) -> str | None:
    ...
    try:
        ...
        logger.warning(f"[UserListener] arecord -l failed: {e}")
        return None

    logger.info(f"[UserListener] ALSA capture devices: {[d['name'] for d in devices]}")
    if not devices:
        return None
    ...
    chosen = pick()
    if chosen:
        logger.info(f"[UserListener] Using ALSA device {chosen['id']} ({chosen['name']})")
        return chosen["id"]
    return None
...
def _transcribe_wav(logger: Any, wav_path: str, transcribe_model: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        logger.exception("[UserListener] transcription request failed")
        return None, str(e)
    finally:
        try:
            proxy.close()
...
def _record_fixed_duration(
    ...
    try:
        ...
        logger.info(f"[UserListener] Recording {dur_int}s (fixed) from -D {dev!r}: {' '.join(cmd)}")
            ...
            if code is not None:
                ...
                if proc.stderr:
                    try:
                        ...
                        pass
                if code != 0:
                    logger.error(f"[UserListener] arecord exited {code}: {err}")
...
def _record_until_silence(
    ...
    *,
    ...
    logger.info(
        f"[UserListener] Recording (silence-end) from -D {dev!r}, "
    ...
    try:
                ...
                if thresh is None:
                    ...
                    if now >= calib_until:
                        if calib_rms:
                            ...
                            thresh = 500.0
                        logger.info(f"[UserListener] Silence-end RMS threshold ≈ {thresh:.1f} (from calibration)")
                    if thresh is None:
                ...
                if not recording:
                    ...
                    if loud:
                        ...
                        if speech_ms >= float(min_speech_ms) and silence_ms >= float(silence_ms_after_speech):
                            logger.info(
                                f"[UserListener] End of speech (~{silence_ms_after_speech}ms quiet), "
        ...
        if proc.stderr:
            try:
                ...
                if err.strip():
                    logger.debug(f"[UserListener] arecord stderr: {err.strip()}")
            except Exception:
                pass

        return bytes(out), None
    except Exception as e:
        logger.exception("[UserListener] silence-end recording failed")
        _terminate_proc(proc)
        return None, str(e)
...
class UserListener(Skill):
    """Record environment audio and return the transcript (JSON on success)."""
    ...
    def guidelines(self):
        return (
            "Listen with the robot microphone, transcribe speech to text. On success the result "
            'message is JSON: {"transcript": "<text>"}. '
            "Modes: (1) Fixed window — set end_on_silence false (default) and duration_seconds (1–30s). "
            "(2) Stop on silence — set end_on_silence true: recording ends after quiet tail "
            "silence_ms_after_speech (default 750ms) once min_speech_ms (default 200ms) of speech was "
            "detected; capped by max_duration_seconds (default 30). Optional tuning: "
            "noise_calibration_ms, speech_rms_multiplier, pre_roll_ms. Requires arecord and proxy."
        )
    ...
    def execute(
        ...
        **_kwargs,
        ...
        self.logger.info(f"[UserListener] Transcript: {text!r}")
        return payload, SkillResult.SUCCESS

    def cancel(self):
        self._cancelled = True
        return "User listener cancel requested"
